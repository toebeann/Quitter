...
It's worth mentioning that I also considered making a system where devs can flag their mods as supporting this kind of edge case (without any additional dependencies), and then having the plugin auto-quit/prompt to quit only when mods which do not flag support are found installed, which would help to raise developer awareness of the issue and _potentially_ lead to more mods addressing these edge cases, but this approach raises some concerns:
