...
Modding is hard. Unity games modded with BepInEx often weren't _meant_ to be modded in this way, and often it is challenging for mods to properly clean up after themselves when the user returns to the main menu. Situations like these can cause all kinds of bugs if a user doesn't realise the issue, up to and including permanently corrupting save files! Quitter is one way<sup><small>[[†]](#-is-there-a-better-way)</small></sup> to avoid these issues.
...
It's worth mentioning we also considered raising developer awareness of the issue by making a system where devs can flag their mods as supporting this kind of edge case (without any additional dependencies for the mod), and then having the plugin auto-quit/prompt to quit only when mods which do not flag support are found installed, but this approach raises some concerns:
- mods can just flag support even if they don't actually take care of these edge cases,
